...
function Chip({ on, onClick, children }) {
  return <button className={`seg-opt ${on?"on":""}`} onClick={onClick}>
    {on && <Icon name="checkSm" size={15} />}{children}
  </button>;
}

function arrToggle(arr, v) { return arr.includes(v) ? arr.filter(x=>x!==v) : [...arr, v]; }

function SetupScreen({ config, setConfig, onLaunch }) {
  const c = config;
  const set = (k, v) => setConfig({ ...c, [k]: v });
  const tog = (k, v) => set(k, arrToggle(c[k], v));
    ...
    <div className="scroll">
      <div className="page">
        ...
        <div style={{ marginBottom: 26 }}>
          <div style={{ display:"inline-flex", alignItems:"center", gap:8, background:"var(--violet-100)",
            color:"var(--violet-700)", padding:"5px 12px", borderRadius:99, fontSize:12, fontWeight:600, marginBottom:14 }}>
          ...
          <p style={{ color:"var(--muted)", fontSize:14.5, marginTop:9, maxWidth:560, lineHeight:1.55 }}>
            Tell the agent who to look for and why they'd care. It researches every account, finds the right contact,
            and drafts a personalized opener — while you watch.
          </p>
        </div>
          ...
          <div className="card card-pad">
            ...
            <div style={{ marginLeft:34 }} className="field">
              ...
              <div className="seg">
                ...
                <form onSubmit={e=>{e.preventDefault(); if(customRole.trim()){ set("roles",[...c.roles, customRole.trim()]); setCustomRole(""); }}}
                  style={{ display:"inline-flex" }}>
                  <input className="input" value={customRole} onChange={e=>setCustomRole(e.target.value)}
                    placeholder="+ Add title" style={{ width:130, padding:"9px 12px", fontSize:13 }} />
                </form>
              </div>
            </div>
          </div>
              ...
              {SIGNAL_DEFS.map(s => {
                ...
                return (
                  <button key={s.key} onClick={()=>tog("signals",s.key)}
                    style={{ textAlign:"left", border:`1px solid ${on?"var(--violet-400)":"var(--line)"}`, background: on?"var(--violet-50)":"var(--surface)",
                      borderRadius:14, padding:"14px 15px", display:"flex", gap:12, alignItems:"flex-start", transition:"all .14s",
                      boxShadow: on?"inset 0 0 0 1px var(--violet-300)":"none", cursor:"pointer" }}>
                    <span style={{ width:34, height:34, borderRadius:9, flex:"none", display:"grid", placeItems:"center",
                      background: on?"var(--accent-grad)":"var(--surface-2)", color: on?"#fff":"var(--muted)" }}><Icon name={s.icon} size={18} /></span>
            ...
            <div style={{ marginLeft:34, display:"flex", flexDirection:"column", gap:22 }}>
              <div className="field">
                ...
                <div style={{ display:"flex", alignItems:"center", gap:18 }}>
                  <input type="range" min="10" max="200" step="5" value={c.targetCount}
                    onChange={e=>set("targetCount", +e.target.value)}
                    style={{ flex:1, accentColor:"var(--violet-600)", maxWidth:420 }} />
                  <div style={{ minWidth:70, textAlign:"center", padding:"8px 12px", borderRadius:10, background:"var(--violet-50)",
                    border:"1px solid var(--violet-200)", fontWeight:680, fontSize:17, color:"var(--violet-700)", fontVariantNumeric:"tabular-nums" }}>{c.targetCount}</div>
                </div>
              </div>
              <div className="field">
      ...
      {/* sticky launch bar */}
      <div style={{ position:"sticky", bottom:0, background:"rgba(255,255,255,.86)", backdropFilter:"blur(10px)",
        borderTop:"1px solid var(--line)", padding:"14px 28px", display:"flex", alignItems:"center", gap:18, zIndex:10 }}>
        ...
        <div style={{ flex:1 }} />
        {!ready && <span style={{ fontSize:12.5, color:"var(--amber-600)", display:"inline-flex", alignItems:"center", gap:6 }}>
          <Icon name="warn" size={14} /> Pick industry, size, role & geography</span>}
...
function SecNum({ n }) {
  return <span style={{ width:24, height:24, borderRadius:8, background:"var(--accent-grad)", color:"#fff",
    display:"grid", placeItems:"center", fontSize:12.5, fontWeight:680, flex:"none" }}>{n}</span>;
}
